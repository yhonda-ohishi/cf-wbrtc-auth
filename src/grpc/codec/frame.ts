 ...
 */
export function decodeFrames(buffer: Uint8Array): { frames: Frame[]; remaining: Uint8Array } {
  const frames: Frame[] = [];
  let offset = 0;
 ...
 */
export function createDataFrame(data: Uint8Array): Frame {
  return {
    flags: FRAME_DATA,
    data,
  };
 ...
 * "key1: value1\r\nkey2: value2\r\n"
 */
export function createTrailerFrame(trailers: Record<string, string>): Frame {
  const lines: string[] = [];
 ...
 */
export function parseTrailers(data: Uint8Array): Record<string, string> {
