 ...
 */
import { encodeFrame, decodeFrames, createDataFrame, createTrailerFrame, parseTrailers, FRAME_DATA, FRAME_TRAILER, } from './frame';
 ...
 */
export function encodeRequest(envelope) {
 ...
 */
export function encodeResponse(envelope) {
    const encoder = new TextEncoder();
    // Encode headers as JSON
    const headersJson = JSON.stringify(envelope.headers);
    const headersBytes = encoder.encode(headersJson);
 ...
 */
export function isErrorResponse(envelope) {
 ...
 */
export function getError(envelope) {
